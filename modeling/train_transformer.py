...
def train_hourly_model(
    ...
    dataset = prepare_hourly_dataset(
        db_path,
        sequence_length=sequence_length,
        pair=pair,
    )
...
def train_execution_model(
    ...
    dataset = prepare_execution_dataset(
        db_path,
        lookback=lookback,
        pair=pair,
    )
...
def main():
    ...
    parser.add_argument('--pair', type=str, default=DEFAULT_PAIR, help='Trading pair to train on (e.g., BTC/USD)')
