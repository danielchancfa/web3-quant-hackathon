...
def compute_daily_features(db_path: Path, pair: str = DEFAULT_PAIR, label_mode: str = 'ternary') -> pd.DataFrame:
    ...
    atr = daily['atr_14'].fillna(method='ffill')
    ...
    daily = daily.fillna(method='ffill').dropna()
...
def prepare_daily_dataset(db_path: Path, sequence_length: int = 60, label_mode: str = 'ternary') -> DailyRegimeDataset:
    daily = compute_daily_features(db_path, label_mode=label_mode)
...
def compute_hourly_features(db_path: Path, sequence_length: int = 24, pair: str = DEFAULT_PAIR) -> Tuple[np.ndarray, np.ndarray]:
    ...
    atr = hourly['atr_14'].fillna(method='ffill')
    upperband = hl2 + multiplier * atr
    lowerband = hl2 - multiplier * atr
...
def prepare_hourly_dataset(db_path: Path, sequence_length: int = 24) -> HourlySignalDataset:
    features, targets = compute_hourly_features(db_path, sequence_length)
...
def prepare_execution_dataset(db_path: Path, lookback: int = 24) -> HourlyExecutionDataset:
    features, targets = compute_execution_features(db_path, lookback)
